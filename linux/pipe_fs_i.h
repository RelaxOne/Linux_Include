...
struct pipe_inode_info {
	struct mutex mutex;		//互斥量，保证整个系统安全
	wait_queue_head_t wait;	//空或者满管道时的读/写等待指针
	/*
	 * nrbufs: 管道中非空管道缓冲区的数量
	 * curbuf: 当前管道缓冲区条目
	 * buffers: 缓冲区的总数
	 */
	unsigned int nrbufs, curbuf, buffers;
	unsigned int readers;	//当前管道读数量
	unsigned int writers;	//当前管道写数量
	unsigned int files;		//当前管道关联的结构体文件数量（受 ->i_lock 保护）
	unsigned int waiting_writers;	//等待当前空间阻塞数量
	unsigned int r_counter;		//读计数器
	unsigned int w_counter;		//写计数器
	struct page *tmp_page;		//缓存释放的页面
	struct fasync_struct *fasync_readers;
	struct fasync_struct *fasync_writers;
	struct pipe_buffer *bufs;	//管道缓冲区的循环数组
	struct user_struct *user;	//创建管道者
};
